...
export default async function handler(
  ...
  const parsed = posts.map((p: any) => ({
    ...
    hairTypeTags: p.hairTypeTags
      ? JSON.parse(p.hairTypeTags as unknown as string)
      : [],
  }));
  ...
  const saved = await prisma.savedItem.findMany({
    where: { userId: user.id, itemType: 'POST', itemId: { in: postIds } },
  });

  const likedSet = new Set(likes.map((l: { postId: number }) => l.postId));
  const savedSet = new Set(saved.map((s: { itemId: number }) => s.itemId));
