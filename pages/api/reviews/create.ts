...
async function handler(
  ...
  if (proProfile) {
    ...
    const currentSum =
      (proProfile.averageRating || 0) * (proProfile.reviewsCount || 0);
  ...
  if (actualDurationMinutes) {
    ...
    if (clientProfile) {
      ...
      const reviews = await prisma.review.findMany({
        where: { clientId: user.id, actualDurationMinutes: { not: null } },
      });
      const durations = reviews.map(
        (r: { actualDurationMinutes: number | null }) =>
          r.actualDurationMinutes || 0
      );
      ...
      await prisma.clientProfile.update({
        where: { userId: user.id },
        data: { typicalCutDurationMinutes: avg },
      });
    }
  }
