...
export default async function handler(
  ...
  const { city, styleTags, hairTypes, minRating, priceMin, priceMax, q, page } =
    req.query;
  ...
  if (q)
    where.OR = [
      ...
      { professional: { bio: { contains: String(q), mode: 'insensitive' } } },
    ];
  ...
  if (styles.length) {
    for (const s of styles)
      profFilters.push({
        professional: { specialties: { contains: String(s) } },
      });
  }
  if (hair.length) {
    for (const h of hair)
      profFilters.push({
        professional: { hairTypesServed: { contains: String(h) } },
      });
  }
  if (minRating)
    profFilters.push({
      professional: { averageRating: { gte: Number(minRating) } },
    });
  if (priceMin || priceMax) {
    // priceRange stored as string like "$30-$60" - do a contains match for priceMin
    if (priceMin)
      profFilters.push({
        professional: { priceRange: { contains: String(priceMin) } },
      });
  }
  ...
  const mapped = results.map((r: any) => ({
    ...
    name: r.name,
    location: r.location,
    specialties: r.professional?.specialties
      ? JSON.parse(r.professional.specialties as unknown as string)
      : [],
    hairTypesServed: r.professional?.hairTypesServed
      ? JSON.parse(r.professional.hairTypesServed as unknown as string)
      : [],
