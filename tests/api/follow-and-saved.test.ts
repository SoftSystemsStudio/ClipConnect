...
test('client can follow/unfollow a pro and save/unsave a post', async () => {
  ...
  // Register PRO
  const regProReq = createRequest({
    method: 'POST',
    body: { email: proEmail, password, name: 'Pro', role: 'PRO' },
  });
  ...
  // Register CLIENT
  const regClientReq = createRequest({
    method: 'POST',
    body: { email: clientEmail, password, name: 'Client', role: 'CLIENT' },
  });
  ...
  const setCookie =
    loginRes._getHeaders()['set-cookie'] ||
    loginRes._getHeaders()['Set-Cookie'];
  const raw = Array.isArray(setCookie) ? setCookie[0] : (setCookie as string);
  const cookie = raw.split(';')[0];

  // Client follows pro
  const followReq = createRequest({
    method: 'POST',
    headers: { cookie },
    body: { professionalId: pro.id },
  });
  ...
  // Toggle follow off
  const followReq2 = createRequest({
    method: 'POST',
    headers: { cookie },
    body: { professionalId: pro.id },
  });
  ...
  const loginProReq = createRequest({
    method: 'POST',
    body: { email: proEmail, password },
  });
  const loginProRes = createResponse();
  await loginHandler(loginProReq as any, loginProRes as any);
  const setCookiePro =
    loginProRes._getHeaders()['set-cookie'] ||
    loginProRes._getHeaders()['Set-Cookie'];
  const rawPro = Array.isArray(setCookiePro)
    ? setCookiePro[0]
    : (setCookiePro as string);
  ...
  const postBody = {
    ...
    caption: 'Pro post',
    styleTags: ['trim'],
    hairTypeTags: ['straight'],
    location: 'City',
  };
  const postReq = createRequest({
    method: 'POST',
    headers: { cookie: cookiePro },
    body: postBody,
  });
  ...
  // Client saves post
  const saveReq = createRequest({
    method: 'POST',
    headers: { cookie },
    body: { itemType: 'POST', itemId: post.id },
  });
